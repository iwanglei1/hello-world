		...
		<table class="table table-border table-bordered table-bg table-hover table-sort table-responsive">
			<thead>
				<tr class="text-c">
					...
					<th width="140">所属手机</th>
					<th width="80">识别码</th>
					<th width="140">发送号码</th>
			...
			<tbody>
			<?php 
			$a = mysqli_query($GLOBALS['link'],"select * from sms where id>0$w order by date desc limit $s,$f");
			$a2 = mysqli_query($GLOBALS['link'],"select count(id) from sms where id>0$w order by id desc");
	...
	while($row = mysqli_fetch_array($a)){
			...
			?>
				<tr class="text-c">
					...
					<td><?php if($row['type']==1){echo '接收';}else{echo '<font color="#ff0000">发送</font>';}?></td>
					<td><?php echo date('Y-m-d-H:i:s',$row['addtime']);?></td>
					
					<td><?php echo '<a title="删除" href="javascript:;" onclick="member_del(this,\''.$row['id'].'\')" class="ml-5" style="text-decoration:none">删除</a>';?></td>
